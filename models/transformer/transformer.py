...
def make_model(d_atom, N=2, d_model=128, h=8, dropout=0.1, 
               ...
               scale_norm=False, init_type='uniform', use_adapter=False, hdim2=0):
    ...
    model = GraphTransformer(
        ...
        Generator(d_model, aggregation_type, n_output, hdim2, leaky_relu_slope, dropout, scale_norm))
...
class GraphTransformer(nn.Module):
    ...
    def gather(self, src, src_mask, adj_matrix, distances_matrix, edges_att):
        return self.generator.merge(self.encode(src, src_mask, adj_matrix, distances_matrix, edges_att),src_mask)
...
class Generator(nn.Module):
    ...
    def __init__(self, d_model, aggregation_type='mean', n_output=1, hdim2=0, 
                 leaky_relu_slope=0.01, dropout=0.0, scale_norm=False):
        super(Generator, self).__init__()
        if hdim2!=0:#flag that we want a second layer with a hidden dimension of size hdim2
            self.proj = []
            self.proj.append(nn.Linear(d_model, hdim2))
            self.proj.append(nn.LeakyReLU(leaky_relu_slope))
            self.proj.append(ScaleNorm(hdim2) if scale_norm else LayerNorm(hdim2))
            self.proj.append(nn.Dropout(dropout))
            self.proj.append(nn.Linear(hdim2, n_output))
            self.proj = torch.nn.Sequential(*self.proj)
        else:
            self.proj = nn.Linear(d_model, n_output)
        self.aggregation_type = aggregation_type

    def merge(self, x, mask):
