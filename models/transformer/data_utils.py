...
def load_data_from_df(dataset_path, add_dummy_node=True, one_hot_formal_charge=False, use_data_saving=True,root=''):
    """Load and featurize data stored in a types file.

    Args:
        dataset_path (str): A path to the types file containing the data. It should have two columns:
                            the first one contains the labels of the compounds,
                            the second one contains the relative PATH to the compounds file.
        ...
        root: root of the relative PATHS in the types file. Defauts to the empty string (current working directory)
    Returns:
    ...
    feature_path = dataset_path.replace('.types', f'{feat_stamp}.p')
    ...
    data_y = data_df.iloc[:, 0].values
    data_x = data_df.iloc[:, 1].values

    if data_y.dtype == np.float64:
        data_y = data_y.astype(np.float32)

    x_all, y_all = load_data_from_files(data_x, data_y, add_dummy_node=add_dummy_node,
                                         one_hot_formal_charge=one_hot_formal_charge,root=root)
...
def load_data_from_files(x_files, labels, add_dummy_node=True, one_hot_formal_charge=False,root=''):
    """Load and featurize data from lists of filename strings and labels.

    Args:
        x_smiles (list[str]): A list of filename strings.
        labels (list[float]): A list of the corresponding labels.
        add_dummy_node (bool): If True, a dummy node will be added to the molecular graph. Defaults to True.
        one_hot_formal_charge (bool): If True, formal charges on atoms are one-hot encoded. Defaults to False.
        root (string): ROOT of the paths present in x_files. Defaults to the current working directory
    ...
    for fname, label in zip(x_files, labels):
        try:
            mol = MolFromMolFile(os.path.join(root,fname))

            afm, adj, dist = featurize_mol(mol, add_dummy_node, one_hot_formal_charge)
            x_all.append([afm, adj, dist])
            y_all.append([label])
        except ValueError as e:
            logging.warning('the FILE ({}) can not be converted to a graph.\nREASON: {}'.format(fname, e))

    return x_all, y_all


def load_data_from_smi_df(dataset_path, add_dummy_node=True, one_hot_formal_charge=False, use_data_saving=False,two_d_only=False, n_out=1):
